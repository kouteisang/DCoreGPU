#include "klist_block.cuh"
#include "gpuutilis.cuh"


__global__ void update_block(int* global_buffer, int* buf_count, int* global_count, 
    ...
    for(int vid = 0; ; vid ++){
        ...
        for(int o_uid = threadIdx.x + o_offset_start; o_uid < o_offset_end; o_uid += blockDim.x){
        ...
        for(int i_uid = threadIdx.x + i_offset_start; i_uid < i_offset_end; i_uid += blockDim.x){
...
void klist_block(G_pointers &p){
    ...
    while(pos < h_kstatus_v_len){
        ...
        while(count < p.num_vtx){
            ...
            update_block<<<BLK_NUMS, BLK_DIM>>>(global_buffer, buf_count, global_count, p.t_in_deg, p.in_adj, p.in_offset, p.t_out_deg, p.out_adj, p.out_offset, p.visit, k, l, p.core);// peel the invalid vertex
    ...
    // for(int index = 0; index < h_kstatus_v.size(); index ++){
    //     cout << "index = " << h_kstatus_v[index] << endl;
    // }

    // std::ofstream file("/home/cheng/DCoreGPU/dataset/enwiki-2024/kstatus.txt");  // 打开文件
    // for(int i = 0; i < h_kstatus_v.size(); i ++){
    //     file << h_kstatus_v[i] << std::endl; 
    // }

    // for(int k = 0; k < 40; k ++){
        // std::ofstream wr("/home/cheng/DCoreGPU/dataset/enwiki-2024/block-"+std::to_string(72)+".txt");
