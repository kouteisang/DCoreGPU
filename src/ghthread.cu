#include "ghthread.cuh"
#include "gpuutilis.cuh"

__global__ void vertex_to_buffer_gthread(int num_vtx, int* global_buffer, int* buf_count, int* visit){
...
__global__ void hindex_out_cal_thread(int* global_buffer, int* buf_count, int* upper, int* core0, int* hindex_out, int* out_adj, int* out_offset, int k){
...
__global__ void gh_update_change_status_out(int* global_buffer, int* buf_count, int* hindex_in, int* hindex_out, int* core, int* out_adj, int* out_offset, int* core0, int k, int * global_done, int* visit){
...
__global__ void gh_update_change_status_in(int* global_buffer, int* buf_count, int* hindex_in, int* hindex_out, int* core, int* in_adj, int* in_offset, int* core0, int k, int * global_done, int* visit){
...
__global__ void update_upper_by_thread(int* global_buffer, int* buf_count, int* hindex_in, int* hindex_out, int* core){
    __shared__ int end;
    __shared__ int* t_global_buffer;
...
__global__ void vertex_to_buffer_by_core0_thread(int k, int* core0, int* global_buffer, int* buf_count, int num_vtx){
...
__global__ void hindex_in_cal_thread(int* global_buffer, int* buf_count, int* upper, int* core0, int* hindex_in, int* in_adj, int* in_offset, int k){
...
__global__ void hin_count_thread_buffer_ghthreadd(int* in_count_num, int* global_buffer, int* buf_count, int* core, int* core0, int* in_adj, int* in_offset, int k){
...
void ghthread_decomposition(G_pointers &p){
    int iterationh = 0;
    int iterationk = 0;
    ...
    while(pos < h_kstatus_v_len){
        ...
        if(pos == 0){
            ...
            while(done){
                ...
                vertex_to_buffer_gthread<<<BLK_NUMS, BLK_DIM>>>(p.num_vtx, global_buffer, buf_count, p.visit); // 将需要改变的放在buffer里面并设置visit = 1 checked

                hindex_out_cal_thread<<<BLK_NUMS, BLK_DIM>>>(global_buffer, buf_count, p.core, core0, hindex_out, p.out_adj, p.out_offset, k);
                hindex_in_cal_thread<<<BLK_NUMS, BLK_DIM>>>(global_buffer, buf_count, p.core, core0, hindex_in, p.in_adj, p.in_offset, k);

                chkerr(cudaMemset(p.visit, 0, sizeof(int) * p.num_vtx));

                gh_update_change_status_out<<<BLK_NUMS, BLK_DIM>>>(global_buffer, buf_count, hindex_in, hindex_out, p.core, p.out_adj, p.out_offset, core0, k, global_done, p.visit);
                gh_update_change_status_in<<<BLK_NUMS, BLK_DIM>>>(global_buffer, buf_count, hindex_in, hindex_out, p.core, p.in_adj, p.in_offset, core0, k, global_done, p.visit);

                update_upper_by_thread<<<BLK_NUMS, BLK_DIM>>>(global_buffer, buf_count, hindex_in, hindex_out, p.core);
        ...
        if(pos + 1 < h_kstatus_v_len && h_kstatus_v[pos+1] != k+1){
            ...
            vertex_to_buffer_by_core0_thread<<<BLK_NUMS, BLK_DIM>>>(k, core0, global_buffer, buf_count, p.num_vtx);

            hin_count_thread_buffer_ghthreadd<<<BLK_NUMS, BLK_DIM>>>(p.in_count_num, global_buffer, buf_count, p.core, core0, p.in_adj, p.in_offset, k);
